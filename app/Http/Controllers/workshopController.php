...
class workshopController extends Controller
{
    ...
    public function googleMapApWorkShop()    {
        // data needed for api
        $URl = 'https://maps.googleapis.com/maps/api/place/textsearch/json?key=AIzaSyAb1qML5aW84D-NJg4bnu3YPoFyNlZ387E&query=car repair+in+egypt&type=car_repair';
            ...
            // Role Save
                ...
                if($check == true)
                {
                    ...
                    if($check_location == true) {
                        ...
                        $car_service->servicetype = 2 ;
                        $check_car_service = $car_service->save();
                        if ($check_car_service == true) {
                            $workshop = new Workshop();
                            $workshop->carservice_id = $car_service->id;
                            $workshop_check = $workshop->save();
                        }
                        else {
    ...
    public function FilterWorkShop(Request $request) {
        ...
        {
            $findWorkShops = Role::where('name' , 'LIKE' , "%{$request->text}%")->where('type' , '2')->where('lock','1')->limit(10)->get();
