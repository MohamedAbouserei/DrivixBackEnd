...
@section('content')
    ...
    <div class="container-fluid">
        ...
        <div  id="SuccessDelete" class="flash-message " style="display: none;width: 50%; margin: auto;box-shadow: 1px 1px 2px #fff , -1px -1px 1px #fff;">
            <p class="alert alert-success text-white" style="text-align: center;"> &nbsp; Product changed status Successfully <i class="fas fa-check-double"></i></p>
        </div>
            ...
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title text-sm-center text-secondary" style="font-size: 14px;" id="exampleModalLabel">Product will  be Locked / unLocked , Are you sure ?</h5>
    ...
    <script>
                ...
                columns: [
                    ...
                    {data: 'updated_at', name: 'updated_at'},
                    {
                        data: 'id', render: function (data , type , row) {
                            console.log(row);
                            edit = '<a href="/get-products/'+ data +'" class="btn btn-sm btn-info text-white"><i class="far fa-eye"></i> </a>';
                            if(row.status === 0) {
                                lock = '<a  class="btn btn-sm btn-danger text-white" data-toggle="modal" data-target="#deleteModal" onclick="openModal('+ data +' , ' + row.status+ ')" > <i class="fas fa-lock"></i></a>';
                            } else {
                                lock = '<a  class="btn btn-sm btn-danger text-white" data-toggle="modal" data-target="#deleteModal" onclick="openModal('+ data +' , ' + row.status +')" > <i class="fas fa-lock-open"></i> </a>';
                            }
                            return edit + '&nbsp;' + lock;
                        }
                    },
        ...
        function openModal(id , type) {
            if (type === 0) {
                ...
                $('#exampleModalLabel').html('Product will be Active');
                $('.changeStatusButtonContent').html('Active');
            }
            else {
                // this model will lock
                 $('#exampleModalLabel').html('Product will be InActive');
